...
void MainWindow::setupSqliteModel()
{
...
void MainWindow::setupSqliteTable()
{
...
void MainWindow::setupSqliteDatabase()
{
...
void MainWindow::populateForEditing(ChangeDbs *editdbs)
{
...
void MainWindow::updateWhereNeeded(ChangeDbs *editdbs)
{
    m_dbContQuery->clear();
...
void MainWindow::editAction()                                                           //Same as "Add" but with prefilled form
{
    ...
    //Populate
    populateForEditing(editdbs);
    if (editdbs->exec() != ChangeDbs::Accepted)
        return;

    //Update where needed
    updateWhereNeeded(editdbs);
...
void MainWindow::copyCredentialsFromTable(QSqlDatabase testDb, int row)
{
...
void MainWindow::checkAction()                                                          //Check status of selected server
{
    ...
    QSqlDatabase testDb = QSqlDatabase::addDatabase(m_dbContModel->index(row, 3).data().toString(),
                                                    "tests");
    copyCredentialsFromTable(testDb, row);
